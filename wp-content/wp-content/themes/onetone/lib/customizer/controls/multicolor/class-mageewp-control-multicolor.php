 ...
 * Multicolor control.
 */
class Mageewp_Control_Multicolor extends WP_Customize_Control {
	 ...
	 * @var string
	 */
	public $type = 'mageewp-multicolor';
	...
	public function enqueue_scripts() {
		...
		if ( ! class_exists( 'Mageewp_Custom_Build' ) || ! Mageewp_Custom_Build::is_custom_build() ) {
			wp_enqueue_script( 'mageewp-multicolor', trailingslashit( Mageewp::$url ) . 'controls/multicolor/multicolor.js', array( 'jquery', 'customize-base', 'wp-color-picker-alpha' ), false, true );
			wp_enqueue_style( 'mageewp-multicolor-css', trailingslashit( Mageewp::$url ) . 'controls/multicolor/multicolor.css', null );
		}
			...
			<# for ( key in data.choices ) { #>
				<# if ( 'irisArgs' !== key ) { #>
					<div class="multicolor-single-color-wrapper">
						...
						<# } #>
						<input {{{ data.inputAttrs }}} id="{{ data.id }}-{{ key }}" type="text" data-palette="{{ data.palette }}" data-default-color="{{ data.default[ key ] }}" data-alpha="{{ data.alpha }}" value="{{ data.value[ key ] }}" class="mageewp-color-control color-picker multicolor-index-{{ key }}" />
					</div>
				<# } #>
			<# } #>
		</div>
