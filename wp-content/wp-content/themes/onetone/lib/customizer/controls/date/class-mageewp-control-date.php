 ...
 */
class Mageewp_Control_Date extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		if ( class_exists( 'Mageewp_Custom_Build' ) ) {
			...
			Mageewp_Custom_Build::register_dependency( 'jquery-ui-datepicker' );
		}

		if ( ! class_exists( 'Mageewp_Custom_Build' ) || ! Mageewp_Custom_Build::is_custom_build() ) {
			wp_enqueue_script( 'mageewp-date', trailingslashit( Mageewp::$url ) . 'controls/date/date.js', array( 'jquery', 'customize-base', 'jquery-ui-datepicker' ), false, true );
			wp_enqueue_style( 'mageewp-date-css', trailingslashit( Mageewp::$url ) . 'controls/date/date.css', null );
		}
	}
