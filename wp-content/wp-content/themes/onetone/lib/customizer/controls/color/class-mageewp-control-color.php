 ...
 */
class Mageewp_Control_Color extends WP_Customize_Control {
	...
	public function enqueue_scripts() {
		...
		if ( ! class_exists( 'Mageewp_Custom_Build' ) || ! Mageewp_Custom_Build::is_custom_build() ) {
			wp_enqueue_script( 'mageewp-color', trailingslashit( Mageewp::$url ) . 'controls/color/color.js', array( 'jquery', 'customize-base', 'wp-color-picker-alpha' ), false, true );
			wp_enqueue_style( 'mageewp-color-css', trailingslashit( Mageewp::$url ) . 'controls/color/color.css', null );
		}
	...
	protected function content_template() {
		...
		<label>
			...
			<# } #>
			<input type="text" {{{ data.inputAttrs }}} data-palette="{{ data.palette }}" data-default-color="{{ data.default }}" data-alpha="{{ data.choices['alpha'] }}" value="{{ data.value }}" class="mageewp-color-control" {{{ data.link }}} />
		</label>
		<?php
	}
}
