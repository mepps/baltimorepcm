 ...
 */
class Mageewp_Control_Gradient extends WP_Customize_Control {
	...
	public function enqueue_scripts() {
		...
		if ( ! class_exists( 'Mageewp_Custom_Build' ) || ! Mageewp_Custom_Build::is_custom_build() ) {
			wp_enqueue_script( 'mageewp-gradient', trailingslashit( Mageewp::$url ) . 'controls/gradient/gradient.js', array( 'jquery', 'customize-base', 'wp-color-picker-alpha' ), false, true );
			wp_enqueue_style( 'mageewp-gradient-css', trailingslashit( Mageewp::$url ) . 'controls/gradient/gradient.css', null );
		}
	...
	protected function content_template() {
		...
		<div class="global">
			<div class="mode">
				<h4><?php esc_attr_e( 'Mode', 'onetone' ); ?></h4>
				<input class="switch-input screen-reader-text" type="radio" value="linear" name="_customize-gradient-{{{ data.id }}}" id="{{ data.id }}linear" <# if ( ! _.isUndefined( data.value.mode ) && 'linear' === data.value.mode ) { #> checked="checked" <# } #>>
					<label class="switch-label switch-label-<# if ( ! _.isUndefined( data.value.mode ) && 'linear' === data.value.mode ) { #>on <# } else { #>off<# } #>" for="{{ data.id }}linear"><span class="dashicons dashicons-minus"></span><span class="screen-reader-text"><?php esc_attr_e( 'Linear', 'onetone' ); ?></span></label>
				</input>
				<input class="switch-input screen-reader-text" type="radio" value="radial" name="_customize-gradient-{{{ data.id }}}" id="{{ data.id }}radial" <# if ( ! _.isUndefined( data.value.mode ) && 'radial' === data.value.mode ) { #> checked="checked" <# } #>>
					<label class="switch-label switch-label-<# if ( ! _.isUndefined( data.value.mode ) && 'radial' === data.value.mode ) { #>on <# } else { #>off<# } #>" for="{{ data.id }}radial"><span class="dashicons dashicons-marker"></span><span class="screen-reader-text"><?php esc_attr_e( 'Radial', 'onetone' ); ?></span></label>
				</input>
			</div>
			<div class="angle">
				<h4><?php esc_attr_e( 'Angle', 'onetone' ); ?></h4>
		...
		<div class="colors">
			<div class="color-start">
				<div class="color">
					<h4><?php esc_attr_e( 'Start Color', 'onetone' ); ?></h4>
					<input type="text" {{{ data.inputAttrs }}} data-palette="{{ data.palette }}" data-default-color="{{ data.default.start.color }}" data-alpha="{{ data.choices['alpha'] }}" value="{{ data.value.start.color }}" class="mageewp-gradient-control-start color-picker" />
				</div>
				<div class="position">
					<h4><?php esc_attr_e( 'Color Stop', 'onetone' ); ?></h4>
					<input type="range" class="position gradient-{{ data.id }}-start" value="{{ data.value.start.position }}" min="0" max="100">
				</div>
			</div>
			<div class="color-end">
				<div class="color">
					<h4><?php esc_attr_e( 'End Color', 'onetone' ); ?></h4>
					<input type="text" {{{ data.inputAttrs }}} data-palette="{{ data.palette }}" data-default-color="{{ data.default.end.color }}" data-alpha="{{ data.choices['alpha'] }}" value="{{ data.value.end.color }}" class="mageewp-gradient-control-end color-picker" />
				</div>
				<div class="position">
					<h4><?php esc_attr_e( 'Color Stop', 'onetone' ); ?></h4>
