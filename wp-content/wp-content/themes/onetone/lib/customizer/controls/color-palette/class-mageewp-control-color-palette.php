 ...
 */
class Mageewp_Control_Color_Palette extends WP_Customize_Control {
	 ...
	 * @var string
	 */
	public $type = 'mageewp-color-palette';
	...
	public function enqueue() {
		...
		if ( ! class_exists( 'Mageewp_Custom_Build' ) || ! Mageewp_Custom_Build::is_custom_build() ) {
			wp_enqueue_script( 'mageewp-color-palette', trailingslashit( Mageewp::$url ) . 'controls/color-palette/color-palette.js', array( 'jquery', 'customize-base', 'jquery-ui-button' ), false, true );
			wp_enqueue_style( 'mageewp-color-palette-css', trailingslashit( Mageewp::$url ) . 'controls/color-palette/color-palette.css', null );
		}
	}
	...
	public function to_json() {
		...
		if ( ! isset( $this->json['choices']['colors'] ) || empty( $this->json['choices']['colors'] ) ) {
			$this->json['choices']['colors'] = Mageewp_Helper::get_material_design_colors( 'primary' );
		}
