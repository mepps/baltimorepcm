			...
			if ( isset( $field['variables'] ) && $field['variables'] && ! empty( $field['variables'] ) ) {
				...
				foreach ( $field['variables'] as $field_variable ) {
					...
					if ( isset( $field_variable['name'] ) ) {
						...
						$variables[ $variable_name ] = Mageewp::get_option( $field['settings'] );
						if ( $variable_callback ) {
							$variables[ $variable_name ] = call_user_func( $field_variable['callback'], Mageewp::get_option( $field['settings'] ) );
						}
					}
				}
			}
