...
class Mageewp_Values {
	 ...
	 * @static
	 * @access public
	 * @param string $config_id The configuration ID. @see Mageewp_Config.
	...
	public static function get_value( $config_id = '', $field_id = '' ) {
		...
		if ( ! isset( Mageewp::$config[ $config_id ] ) ) {
			$config_id = 'global';
		}

		if ( 'theme_mod' === Mageewp::$config[ $config_id ]['option_type'] ) {

			// We're using theme_mods so just get the value using get_theme_mod.
			$default_value = null;
			if ( isset( Mageewp::$fields[ $field_id ] ) && isset( Mageewp::$fields[ $field_id ]['default'] ) ) {
				$default_value = Mageewp::$fields[ $field_id ]['default'];
			}
			$value = get_theme_mod( $field_id, $default_value );
			return apply_filters( 'options-framework/values/get_value', $value, $field_id );
		}

		if ( 'option' === Mageewp::$config[ $config_id ]['option_type'] ) {

			// We're using options.
			if ( '' !== Mageewp::$config[ $config_id ]['option_name'] ) {

				// Options are serialized as a single option in the db.
				// We'll have to get the option and then get the item from the array.
				
				
				$options = get_option( Mageewp::$config[ $config_id ]['option_name'] );
					

				if ( ! isset( Mageewp::$fields[ $field_id ] ) && isset( Mageewp::$fields[ Mageewp::$config[ $config_id ]['option_name'] . '[' . $field_id . ']' ] ) ) {
					$field_id = Mageewp::$config[ $config_id ]['option_name'] . '[' . $field_id . ']';
				}
				$setting_modified = str_replace( ']', '', str_replace( Mageewp::$config[ $config_id ]['option_name'] . '[', '', $field_id ) );

				$default_value = ( isset( Mageewp::$fields[ $field_id ] ) && isset( Mageewp::$fields[ $field_id ]['default'] ) ) ? Mageewp::$fields[ $field_id ]['default'] : '';
			...
			if(isset($fields[ $field_id ]['default'])){
				$value = get_option( $field_id, Mageewp::$fields[ $field_id ]['default'] );
				return apply_filters( 'options-framework/values/get_value', $value, $field_id );
			}
