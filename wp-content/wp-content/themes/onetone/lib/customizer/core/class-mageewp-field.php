...
class Mageewp_Field {
	...
	/**
	 * The ID of the mageewp_config we're using.
	 *
	 * @see Mageewp_Config
	 * @access protected
	 * @var string
	 */
	protected $mageewp_config = 'global';
	 ...
	 * @var string
	 */
	protected $type = 'mageewp-generic';
	 ...
	 * Then it adds the field to Mageewp::$fields.
	 *
	 * @access public
	...
	public function __construct( $config_id = 'global', $args = array() ) {
		...
		if ( isset( $args['setting'] ) && ! empty( $args['setting'] ) && ( ! isset( $args['settings'] ) || empty( $args['settings'] ) ) ) {
			...
			unset( $args['setting'] );
			error_log( 'Mageewp: Typo found in field ' . $args['settings'] . ' ("setting" instead of "settings").' );
		}
		...
		$args['mageewp_config'] = $config_id;

		$this->mageewp_config = ( '' === $config_id ) ? 'global' : trim( esc_attr( $config_id ) );

		// Get defaults from the class.
		$defaults = get_class_vars( __CLASS__ );

		// Get the config arguments, and merge them with the defaults.
		$config_defaults = ( isset( Mageewp::$config['global'] ) ) ? Mageewp::$config['global'] : array();
		if ( 'global' !== $this->mageewp_config && isset( Mageewp::$config[ $this->mageewp_config ] ) ) {
			$config_defaults = Mageewp::$config[ $this->mageewp_config ];
		}
		...
		$whitelisted = apply_filters( "options-framework/{$this->mageewp_config}/fields/properties_whitelist", array(
	...
	protected function set_field( $whitelisted_properties = array() ) {
		...
		Mageewp::$fields[ $this->settings ] = $args;
	...
	/**
	 * Escape $mageewp_config.
	 *
	 * @access protected
	 */
	protected function set_mageewp_config() {

		$this->mageewp_config = esc_attr( $this->mageewp_config );
	...
	protected function set_settings() {
		...
		if ( ! is_array( $this->settings ) ) {
			$this->settings = array(
				'mageewp_placeholder_setting' => $this->settings,
			);
		}
		$settings = array();
		...
		if ( isset( $this->settings['mageewp_placeholder_setting'] ) ) {
			$this->settings = $this->settings['mageewp_placeholder_setting'];
		}
	...
	protected function set_active_callback() {
		...
		if ( ! empty( $this->required ) ) {
			$this->active_callback = array( 'Mageewp_Active_Callback', 'evaluate' );
			return;
		}
	...
	protected function set_sanitize_callback() {
		...
		$default_callbacks = array(
			'mageewp-multicheck'       => array( 'Mageewp_Sanitize_Values', 'multicheck' ),
			'mageewp-typography'       => array( 'Mageewp_Sanitize_Values', 'typography' ),
		);
	...
	protected function set_output() {
		...
		foreach ( $this->output as $output ) {
			if ( ! isset( $output['element'] ) || ( ! isset( $output['property'] ) && ! in_array( $this->type, array( 'mageewp-typography', 'mageewp-background' ), true ) ) ) {
				continue;
			}
