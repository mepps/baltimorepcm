...
/**
 * Our main Mageewp_Control object
 */
class Mageewp_Custom_Build {
	...
	public static function is_custom_build() {
		if ( null === self::$is_custom_build ) {
			if ( file_exists( Mageewp::$path . '/build.min.js' ) && file_exists( Mageewp::$path . '/build.min.css' ) ) {
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script( 'mageewp-build', trailingslashit( Mageewp::$url ) . 'build.min.js', self::$dependencies );
		wp_enqueue_style( 'mageewp-build', trailingslashit( Mageewp::$url ) . 'build.min.css', null );
