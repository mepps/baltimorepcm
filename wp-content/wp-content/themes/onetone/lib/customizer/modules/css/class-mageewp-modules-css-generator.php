 ...
 * Handles CSS output.
 */
final class Mageewp_Modules_CSS_Generator {
	...
	private function __construct() {
		if ( is_null( self::$google_fonts ) ) {
			self::$google_fonts = Mageewp_Fonts::get_google_fonts();
		}
		if ( is_null( self::$backup_fonts ) ) {
			self::$backup_fonts = Mageewp_Fonts::get_backup_fonts();
		}
	}
	...
	public static function css( $field ) {
		...
		self::$value = Mageewp_Values::get_sanitized_field_value( $field );

		// Find the class that will handle the outpout for this field.
		$classname = 'Mageewp_Output';
		$field_output_classes = apply_filters( "options-framework/{$field['mageewp_config']}/output/control-classnames", array(
			'mageewp-background'  => 'Mageewp_Output_Field_Background',
			'mageewp-dimensions'  => 'Mageewp_Output_Field_Dimensions',
			'mageewp-typography'  => 'Mageewp_Output_Field_Typography',
			'mageewp-multicolor'  => 'Mageewp_Output_Field_Multicolor',
		) );
		if ( array_key_exists( self::$field_type, $field_output_classes ) ) {
			$classname = $field_output_classes[ self::$field_type ];
		}
		$obj = new $classname( $field['mageewp_config'], self::$output, self::$value );
