 ...
 * Output overrides.
 */
class Mageewp_Output_Property_Background_Position extends Mageewp_Output_Property {
	...
	protected function process_value() {
		...
		if ( false !== strpos( $this->value, ' ' ) ) {
			...
			if ( ! in_array( $x, $x_dimensions, true ) ) {
				$x = Mageewp_Sanitize_Values::css_dimension( $x );
			}
			if ( ! in_array( $y, $y_dimensions, true ) ) {
				$y = Mageewp_Sanitize_Values::css_dimension( $y );
			}
			$this->value = $x . ' ' . $y;
			return;
		}
