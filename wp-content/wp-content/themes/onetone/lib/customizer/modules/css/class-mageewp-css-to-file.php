...
class Mageewp_CSS_To_File {
	...
	protected function get_path( $context = '' ) {
		...
		$paths = array(
			'file'   => wp_normalize_path( $upload_dir['basedir'] . '/mageewp-css/styles.css' ),
			'folder' => wp_normalize_path( $upload_dir['basedir'] . '/mageewp-css' ),
		);
	...
	public function get_url() {
		...
		$upload_dir = wp_upload_dir();
		return esc_url_raw( $upload_dir['baseurl'] . '/mageewp-css/styles.css' );
	...
	public function write_file() {
		...
		foreach ( $configs as $config_id => $args ) {
			...
			$css[ $config_id ] = apply_filters( "options-framework/{$config_id}/dynamic_css", Mageewp_Modules_CSS::loop_controls( $config_id ) );
		}
		$css = implode( $css, '' );
		...
		if ( ! $write_file ) {
			$this->fallback = true;
			set_transient( 'mageewp_css_write_to_file_failed', true, HOUR_IN_SECONDS );
		}
		return $write_file;
