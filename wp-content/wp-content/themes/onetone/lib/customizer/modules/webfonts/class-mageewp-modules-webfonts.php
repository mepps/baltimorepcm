 ...
 */
class Mageewp_Modules_Webfonts {
	...
	protected function __construct() {
		...
		include_once wp_normalize_path( dirname( __FILE__ ) . '/class-mageewp-fonts.php' );
		include_once wp_normalize_path( dirname( __FILE__ ) . '/class-mageewp-fonts-google.php' );
	 ...
	 */
	public function run() {
		$this->fonts_google = Mageewp_Fonts_Google::get_instance();
	...
	protected function init() {
		...
		foreach ( Mageewp::$config as $config_id => $args ) {
			$method = $this->get_method( $config_id );
			$classname = 'Mageewp_Modules_Webfonts_' . ucfirst( $method );
	...
	protected function maybe_fallback_to_link() {
		...
		$fallback_to_link = get_transient( 'mageewp_googlefonts_fallback_to_link' );
	...
	public function loop_fields( $config_id ) {
		foreach ( Mageewp::$fields as $field ) {
			if ( isset( $field['mageewp_config'] ) && $config_id !== $field['mageewp_config'] ) {
				continue;
			}
