 ...
 */
final class Mageewp_Modules_Webfonts_Embed {
	...
	/**
	 * The Mageewp_Modules_Webfonts object.
	 *
	 * @access protected
	 * @var object
	 */
	...
	/**
	 * The Mageewp_Fonts_Google object.
	 *
	 * @access protected
	 * @var object
	 */
	 ...
	 * @param object $webfonts    The Mageewp_Modules_Webfonts object.
	 * @param object $googlefonts The Mageewp_Fonts_Google object.
	...
	public function inline_css() {
		wp_add_inline_style( 'mageewp-styles', $this->embed_css() );
	}

	/**
	 * Embeds the CSS from googlefonts API inside the Mageewp output CSS.
	 *
	 * @access public
	...
	public function embed_css( $css = '' ) {
		...
		$link_obj = new Mageewp_Modules_Webfonts_Link( $this->config_id, $this->webfonts, $this->googlefonts );
	...
	protected function get_url_contents( $url = '' ) {
		...
		// The transient name.
		$transient_name = 'mageewp_googlefonts_contents_' . md5( $url );
		...
		if ( false === $data ) {
			...
			if ( is_wp_error( $response ) ) {
				set_transient( 'mageewp_googlefonts_fallback_to_link', 'yes', HOUR_IN_SECONDS );
				return false;
			}
			...
			set_transient( 'mageewp_googlefonts_fallback_to_link', 'no', DAY_IN_SECONDS );
		}
