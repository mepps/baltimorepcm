 ...
 */
final class Mageewp_Fonts_Google {

	/**
	 * The Mageewp_Fonts_Google instance.
	...
	private function __construct() {
		...
		$this->google_fonts = Mageewp_Fonts::get_google_fonts();
	 ...
	 *
	 * @return object Mageewp_Fonts_Google
	 */
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new Mageewp_Fonts_Google();
		}
	...
	public function generate_google_font( $args ) {
		...
		if ( isset( $args['type'] ) && 'mageewp-typography' === $args['type'] ) {

			// Get the value.
			$value = Mageewp_Values::get_sanitized_field_value( $args );
			...
			if ( ! Mageewp_Fonts::is_google_font( $value['font-family'] ) ) {
				return;
			}
			...
			if ( isset( $value['subsets'] ) ) {
				...
				// Add the subset directly to the array of subsets in the Mageewp_GoogleFonts_Manager object.
