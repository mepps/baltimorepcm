 ...
 */
class Mageewp_Modules_Custom_Sections {
	...
	public function set_section_types( $section_types ) {
		...
		$new_types = array(
			'mageewp-default'  => 'Mageewp_Sections_Default_Section',
			'mageewp-expanded' => 'Mageewp_Sections_Expanded_Section',
			'mageewp-nested'   => 'Mageewp_Sections_Nested_Section',
		);
	...
	public function set_panel_types( $panel_types ) {
		...
		$new_types = array(
			'mageewp-nested' => 'Mageewp_Panels_Nested_Panel',
		);
	...
	public function include_sections_and_panels() {
		...
		foreach ( $section_types as $id => $class ) {
			if ( ! class_exists( $class ) ) {
				$path = wp_normalize_path( $folder_path . 'class-mageewp-sections-' . $id . '-section.php' );
				if ( file_exists( $path ) ) {
					include_once $path;
					continue;
				}
				$path = str_replace( 'class-mageewp-sections-mageewp-', 'class-mageewp-sections-', $path );
				if ( file_exists( $path ) ) {
					include_once $path;
				}
			}
		...
		foreach ( $panel_types as $id => $class ) {
			if ( ! class_exists( $class ) ) {
				$path = wp_normalize_path( $folder_path . 'class-mageewp-panels-' . $id . '-panel.php' );
				if ( file_exists( $path ) ) {
					include_once $path;
					continue;
				}
				$path = str_replace( 'class-mageewp-panels-mageewp-', 'class-mageewp-panels-', $path );
				if ( file_exists( $path ) ) {
					include_once $path;
				}
			}
	...
	public function enqueue_scrips() {
		...
		wp_enqueue_style( 'mageewp-custom-sections', trailingslashit( Mageewp::$url ) . 'modules/custom-sections/sections.css' );
		wp_enqueue_script( 'mageewp-custom-sections', trailingslashit( Mageewp::$url ) . 'modules/custom-sections/sections.js', array( 'jquery', 'customize-base', 'customize-controls' ) );
