jQuery( document ).ready( function() {
	...
	wp.customize.section.each( function( section ) {
		...
		var link = '<a href="#" class="mageewp-reset-section" data-reset-section-id="' + section.id + '">' + hooResetButtonLabel['reset-with-icon'] + '</a>';

		jQuery( link ).appendTo( '#sub-accordion-section-' + section.id + ' .customize-section-title > h3' );

	});

	jQuery( 'a.mageewp-reset-section' ).on( 'click', function() {
